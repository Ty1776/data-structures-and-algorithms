...
const count = (target, input) => {
  // Solution code here...
  return input.flat().filter(x => x === target).length;
};
...
const totalSum = (input) => {
  // Solution code here...
  return input.flat().reduce((accumulator, value) => accumulator + value, 0);
};
...
const divisibleByFiveTwoToThePower = (input) => {
  // Solution code here...
  return input.map(arr => arr.filter(num => typeof num === 'number' && num % 5 === 0).map(num => 2 ** num));
};
...
let findShortest = (data) => {
  ...
  return data.reduce((shortest, character) => {
    if (shortest.height === undefined || parseInt(character.height) < parseInt(shortest.height)) {
      shortest = character;
    }
    return shortest;
  }, {}).name;
