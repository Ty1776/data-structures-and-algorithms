...
let biggerThanLuke = (arr) => {
  ...
  if (!luke) return '';
  let lukesMass = luke.mass;
  let filtered = arr.filter(character => parseInt(character.mass) > parseInt(lukesMass));
  let namesArr = filtered.map(character => character.name);
  let joined = namesArr.join(' - ');
  return joined;
};
